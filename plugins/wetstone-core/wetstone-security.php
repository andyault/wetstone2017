...
function wetstone_prevent_wp_login() {
	...
	if($pagenow == 'wp-login.php' && (!$action || ($action && !in_array($action, array('logout', 'lostpassword'))))) { //, 'rp', 'resetpass'))))) {
		...
		// Redirect
		wp_redirect(add_query_arg($_GET, get_permalink(get_page_by_path($wetstone_redirect_slug))));
...
function wetstone_remove_admin_bar() {
	if(!current_user_can('administrator') && !is_admin())
		show_admin_bar(false);
}

add_action('after_setup_theme', 'wetstone_remove_admin_bar');