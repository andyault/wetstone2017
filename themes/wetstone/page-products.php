	...
	<div id="product-preview-slides" class="product-preview-slides carousel-slides">
		<?php
			...
			foreach($posts as $post) {
				setup_postdata($post);
				if($post->ID >= 612 && $post->ID <= 634 || $post->ID == 1054 || $post->ID == 116 || $post->ID == 1815 || $post->ID == 1667) { // IDs of unwanted products
					if($post->ID == 633) {
		...
		foreach($grouped as $group => $posts) { ?>
			...
			<div class="page-list flex flex-center">
				<?php
					foreach($posts as $post) {
						setup_postdata($post);
						if($post->ID >= 612 && $post->ID <= 634 || $post->ID == 1054 || $post->ID == 116 || $post->ID == 1815) { // IDs of unwanted products
